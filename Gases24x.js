...
const c1 = 'rgba(109, 128, 255, 0.90)';
const c2 = 'rgba(255, 142, 18, 0.95)';
const c3 = 'rgba(168,212,171,0.98)'

const ALLOWED_GASES = ['A','B','C'];

const c1P = 'rgbs(109, 128, 255,1)';
const c2P = 'rgbs(255, 142, 18,1)';
...
for (let i = 0; i < 15; i++){
  ...
  function veloAlpha(vx,vy,color) {
    ...
    else if (v>vCeil){v=vCeil}

    let x = - AlSt * ((v - vFloor)/(vCeil-vFloor))**AlPow + AlMax;
    
    // Use a regular expression to parse the RGBA string
...
let SBin = '   Speed (arbitrary units)   ';
...
function drawScene(){
  ...
  var layout = {
    ...
    xaxis: {range: [0, vMax]},
    yaxis: {range: [0, 1]},  
    paper_bgcolor: 'rgba(207,221,199,.0)',
    plot_bgcolor: 'rgba(220,220,224,0)', 
    autosize: false,
  ...
  for (let i = 0; i < particles.length; i++){
    ...
    if (particles[i].Species == 'A'){
      ...
      if (HideA){
        c = cH;
      } else { c = veloAlpha(particles[i].vx,particles[i].vy,c1)//color(c1);
        drawParticle(particles[i].x, particles[i].y, particles[i].r, c);}

    } else if (particles[i].Species == 'B'){

        c = veloAlpha(particles[i].vx,particles[i].vy,c2);
        drawParticle(particles[i].x, particles[i].y, particles[i].r, c);
    } else if (particles[i].Species == 'C'){

      c = veloAlpha(particles[i].vx,particles[i].vy,c3);
      drawParticle(particles[i].x, particles[i].y, particles[i].r, c);
  }
...
function drawParticleV(x, y, vx, vy, r, c) {
  noStroke();
  fill(c);
  ellipse(x, y, 2 * r, 2 * r);
